...
public class CSBalancing extends CommandBase {
    ...
    public CSBalancing(boolean angleCorrection) {
    ...
    public void initialize() {
        Drivetrain.getInstance().drive(0, -2.2, 0, true);
    }
    ...
    public void execute() {
        ...
        Gyroscope.getPigeonInstance().getYawPitchRoll(ypr_deg);

        this.maxAngle = Math.max(this.maxAngle, ypr_deg[gyroRelevantAxis]);

        //PID Angle Correction
        if(this.angleCorrection) {
            double linearControl = Math.pow(Gyroscope.getPigeonInstance().getYaw()/360, 3)*0.1;
